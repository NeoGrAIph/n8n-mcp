...
8. **Deployment** (if n8n API configured)
   ...
   - `n8n_workflow_test({workflowId})` - Test workflow execution
...
**With MCP:** "Everything just... worked. Instead of guessing, I could ask `n8n_node_get()` and get exactly what I needed - not a 100KB JSON dump, but the actual properties that matter. What took 45 minutes now takes 3 minutes."
...
- `get_node_essentials`, `get_node_info`, `get_node_documentation` → **`n8n_node_get`** (use `detail` and `mode`)
- `list_nodes` → **`n8n_nodes_search`**
...
- **`n8n_nodes_search`** - Full-text search across all nodes. Use `includeExamples: true` for real-world configurations
- **`n8n_node_get`** - Unified node information tool with multiple modes (v2.26.0):
...
- **`n8n_node_validate`** - Unified node validation (v2.26.0):
...
- **`n8n_workflow_json_validate`** - Complete workflow validation including AI Agent validation
- **`n8n_templates_search`** - Unified template search (v2.26.0):
...
- **`n8n_template_get`** - Get complete workflow JSON (modes: nodes_only, structure, full)

### n8n Management Tools (Requires API Configuration)
These tools require `N8N_API_URL` and `N8N_API_KEY` in your configuration.

#### Workflow Management
- **`n8n_workflow_create`** - Create new workflows with nodes and connections
- **`n8n_workflow_get`** - Unified workflow retrieval (v2.26.0):
...
- **`n8n_workflow_update_full`** - Update entire workflow (complete replacement)
- **`n8n_workflow_update_partial`** - Update workflow using diff operations
- **`n8n_workflow_delete`** - Delete workflows permanently
- **`n8n_workflows_list`** - List workflows with filtering and pagination
- **`n8n_workflow_validate`** - Validate workflows in n8n by ID
- **`n8n_workflow_autofix`** - Automatically fix common workflow errors
- **`n8n_template_deploy`** - Deploy templates from n8n.io directly to your instance with auto-fix
...
- **`n8n_workflow_test`** - Test/trigger workflow execution:
