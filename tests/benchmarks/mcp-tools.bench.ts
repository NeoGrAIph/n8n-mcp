...
describe('MCP Tool Performance (Production Database)', () => {
  ...
  /**
   * n8n_nodes_search - Most frequently used tool for node discovery
   *
   * This measures:
   ...
   */
  bench('n8n_nodes_search - common query (http)', async () => {
  ...
  bench('n8n_nodes_search - AI agent query (slack message)', async () => {
  ...
  /**
   * n8n_node_validate (mode=full) - Configuration validation
   *
   * This measures:
   * - Schema lookup
   ...
   */
  bench('n8n_node_validate (mode=full) - HTTP Request (minimal)', async () => {
    const node = await repository.getNodeByType('n8n-nodes-base.httpRequest');
    if (node && node.properties) {
  ...
  bench('n8n_node_validate (mode=full) - HTTP Request (with params)', async () => {
    const node = await repository.getNodeByType('n8n-nodes-base.httpRequest');
    if (node && node.properties) {
      EnhancedConfigValidator.validateWithMode(
        'n8n-nodes-base.httpRequest',
