...
describe('MCP Error Handling', () => {
  ...
  describe('Tool-Specific Errors', () => {
    describe('Node Discovery Errors', () => {
      ...
      it('should handle invalid search mode', async () => {
        try {
          await client.callTool({ name: 'n8n_nodes_search', arguments: {
            query: 'test',
      ...
      it('should handle empty search query', async () => {
        try {
          ...
          expect(error).toBeDefined();
          expect(error.message).toContain("n8n_nodes_search: Validation failed:");
    ...
    describe('Validation Errors', () => {
      // v2.26.0: n8n_node_validate_operation consolidated into n8n_node_validate
      it('should handle invalid validation profile', async () => {
        try {
          await client.callTool({ name: 'n8n_node_validate', arguments: {
      ...
      it('should handle malformed workflow structure', async () => {
        try {
          await client.callTool({ name: 'n8n_workflow_json_validate', arguments: {
            workflow: {
          ...
          expect(error).toBeDefined();
          expect(error.message).toContain("n8n_workflow_json_validate: Validation failed:");
  ...
  describe('Large Payload Handling', () => {
    ...
    it('should handle many concurrent requests', async () => {
      ...
      for (let i = 0; i < requestCount; i++) {
        promises.push(
          client.callTool({ name: 'n8n_nodes_search', arguments: {
  ...
  describe('Error Recovery', () => {
    it('should continue working after errors', async () => {
      ...
      // Should still work
      const response = await client.callTool({ name: 'n8n_nodes_search', arguments: { query: 'webhook', limit: 1 } });
      expect(response).toBeDefined();
    });

    it('should handle mixed success and failure', async () => {
      const promises = [
        client.callTool({ name: 'n8n_nodes_search', arguments: { query: 'webhook', limit: 5 } }),
        client.callTool({ name: 'n8n_node_get', arguments: { nodeType: 'invalid' } }).catch(e => ({ error: e })),
        client.callTool({ name: 'n8n_tools_documentation', arguments: {} }),
        client.callTool({ name: 'n8n_nodes_search', arguments: { query: '' } }).catch(e => ({ error: e })),
