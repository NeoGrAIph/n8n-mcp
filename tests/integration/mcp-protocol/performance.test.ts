...
describe('MCP Performance Tests', () => {
  ...
  beforeEach(async () => {
    ...
    const searchResponse = await client.callTool({ name: 'n8n_nodes_search', arguments: { query: 'http', limit: 1 } });
  ...
  describe('Response Time Benchmarks', () => {
    ...
    it('should handle search operations efficiently', async () => {
      ...
      for (let i = 0; i < iterations; i++) {
        await client.callTool({ name: 'n8n_nodes_search', arguments: { query: 'http', limit: 10 } });
      }

      const duration = performance.now() - start;
      const avgTime = duration / iterations;

      console.log(`Average response time for n8n_nodes_search: ${avgTime.toFixed(2)}ms`);
      console.log(`Environment: ${process.env.CI ? 'CI' : 'Local'}`);
    ...
    it('should perform searches efficiently', async () => {
      ...
      for (let i = 0; i < iterations; i++) {
        for (const query of searches) {
          await client.callTool({ name: 'n8n_nodes_search', arguments: { query } });
        }
      }
    ...
    it('should retrieve node info quickly', async () => {
      ...
      for (const nodeType of nodeTypes) {
        await client.callTool({ name: 'n8n_node_get', arguments: { nodeType } });
      }

      const duration = performance.now() - start;
      const avgTime = duration / nodeTypes.length;

      console.log(`Average response time for n8n_node_get: ${avgTime.toFixed(2)}ms`);
      console.log(`Environment: ${process.env.CI ? 'CI' : 'Local'}`);
  ...
  describe('Concurrent Request Performance', () => {
    it('should handle concurrent requests efficiently', async () => {
      ...
      for (let i = 0; i < concurrentRequests; i++) {
        promises.push(
          client.callTool({ name: 'n8n_nodes_search', arguments: { query: 'http', limit: 5 } })
        );
      }
    ...
    it('should handle mixed concurrent operations', async () => {
      const operations = [
        { tool: 'n8n_nodes_search', params: { query: 'http', limit: 10 } },
        { tool: 'n8n_nodes_search', params: { query: 'webhook' } },
        { tool: 'n8n_tools_documentation', params: {} },
        { tool: 'n8n_node_get', params: { nodeType: 'nodes-base.httpRequest' } },
        { tool: 'n8n_node_get', params: { nodeType: 'nodes-base.webhook' } }
      ];
  ...
  describe('Large Data Performance', () => {
    it('should handle large search results efficiently', async () => {
      ...
      if (response.content && Array.isArray(response.content) && response.content[0]) {
        ...
        try {
          ...
          // n8n_nodes_search returns an object with results property
    ...
    it('should handle large workflow validation efficiently', async () => {
      ...
      const response = await client.callTool({ name: 'n8n_workflow_json_validate', arguments: {
  ...
  describe('Memory Efficiency', () => {
    ...
    it('should release memory after large operations', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        await client.callTool({ name: 'n8n_nodes_search', arguments: { query: 'n8n', limit: 200 } });
        await client.callTool({ name: 'n8n_node_get', arguments: {
  ...
  describe('Scalability Tests', () => {
    it('should maintain performance with increasing load', async () => {
      ...
      for (const load of loadLevels) {
        ...
        for (let i = 0; i < load; i++) {
          promises.push(
            client.callTool({ name: 'n8n_nodes_search', arguments: { query: 'http', limit: 1 } })
          );
        }
    ...
    it('should handle burst traffic', async () => {
      ...
      for (let i = 0; i < burstSize; i++) {
        ...
        switch (operation) {
          case 0:
            promises.push(client.callTool({ name: 'n8n_nodes_search', arguments: { query: 'http', limit: 5 } }));
            break;
          case 1:
            promises.push(client.callTool({ name: 'n8n_nodes_search', arguments: { query: 'test' } }));
            break;
          case 2:
            promises.push(client.callTool({ name: 'n8n_tools_documentation', arguments: {} }));
            break;
          case 3:
            promises.push(client.callTool({ name: 'n8n_node_get', arguments: { nodeType: 'nodes-base.set' } }));
            break;
        }
      }
  ...
  describe('Critical Path Optimization', () => {
    it('should optimize search performance', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        await client.callTool({ name: 'n8n_nodes_search', arguments: { query: 'http', limit: 1 } });
      }
      ...
      for (let i = 0; i < iterations; i++) {
        const start = performance.now();
        await client.callTool({ name: 'n8n_nodes_search', arguments: { query: 'http', limit: 20 } });
      ...
      console.log(`n8n_nodes_search performance - Avg: ${avgTime.toFixed(2)}ms, Min: ${minTime.toFixed(2)}ms, Max: ${maxTime.toFixed(2)}ms`);
      console.log(`Environment: ${process.env.CI ? 'CI' : 'Local'}`);
    ...
    it('should handle varied search queries efficiently', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        await client.callTool({ name: 'n8n_nodes_search', arguments: { query: 'test' } });
      }
      ...
      console.log(`n8n_nodes_search average performance: ${avgTime.toFixed(2)}ms`);
      console.log(`Environment: ${process.env.CI ? 'CI' : 'Local'}`);
  ...
  describe('Stress Tests', () => {
    ...
    it('should recover from performance degradation', async () => {
      ...
      for (let i = 0; i < 200; i++) {
        heavyPromises.push(
          client.callTool({ name: 'n8n_workflow_json_validate', arguments: {
            workflow: {
