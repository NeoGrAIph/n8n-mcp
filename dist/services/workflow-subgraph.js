...
exports.buildSubWorkflow = buildSubWorkflow;
...
function collectUpstream(startNodes, inEdges) {
...
function collectDownstream(startNodes, outEdges, endNodes) {
    const visited = new Set();
    const queue = [];
    for (const node of startNodes) {
        queue.push(node);
...
function filterConnections(connections, selected) {
    const filtered = {};
    for (const [sourceName, outputs] of Object.entries(connections || {})) {
...
function getRootNodes(selected, inEdges) {
    const roots = [];
...
function createExecuteWorkflowTrigger(existingNames) {
...
function collectNodeReferences(value, references) {
...
function applyExpressionReferences(selected, graph, includeUpstream, warnings) {
...
function buildSubWorkflow(input) {
    const warnings = [];
    ...
    for (const nodeName of Array.from(selected)) {
        const node = graph.nodeByName.get(nodeName);
        if (node && (0, node_type_utils_1.isTriggerNode)(node.type)) {
    ...
    const existingNames = new Set(selectedNodes.map(node => node.name));
...
//# sourceMappingURL=workflow-subgraph.js.map