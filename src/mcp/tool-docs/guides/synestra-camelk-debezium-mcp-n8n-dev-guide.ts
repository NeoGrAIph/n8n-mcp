...
export const synestraCamelKDebeziumMcpN8nDevGuide: ToolDocumentation = {
  name: 'synestra_camelk_debezium_mcp_n8n_dev_guide',
  category: 'guides',
  essentials: {
    description:
      'Synestra dev guide: how Camel K + Debezium, n8n-sync-controller, and mcp-n8n work together, what each component does, and how to safely iterate on workflow files with MCP tools.',
    keyParameters: [],
    example:
      'Use n8n_tools_documentation({topic: "synestra_camelk_debezium_mcp_n8n_dev_guide"}) to access this guide',
...
If the change is consistently overwritten or never appears in the API, treat it as "not synergistic" and debug scheduling/mounts/controller logs.`,
    ...
    returns:
      'Markdown guide describing how Synestra dev runs Camel K + Debezium alongside mcp-n8n, with safe agent workflows and operational checks.',
    examples: [
