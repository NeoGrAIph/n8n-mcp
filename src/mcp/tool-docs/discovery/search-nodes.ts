...
export const searchNodesDoc: ToolDocumentation = {
  ...
  essentials: {
    ...
    example: 'n8n_search_nodes({query: "webhook"})',
  ...
  full: {
    ...
    examples: [
      'n8n_search_nodes({query: "webhook"}) - Returns Webhook node as top result',
      'n8n_search_nodes({query: "database"}) - Returns MySQL, Postgres, MongoDB, Redis, etc.',
      'n8n_search_nodes({query: "google sheets", mode: "AND"}) - Requires both words',
      'n8n_search_nodes({query: "slak", mode: "FUZZY"}) - Finds Slack despite typo',
      'n8n_search_nodes({query: "http api"}) - Finds HTTP Request, GraphQL, REST nodes',
      'n8n_search_nodes({query: "transform data"}) - Finds Set, Code, Function, Item Lists nodes'
    ],
    useCases: [
    ...
    bestPractices: [
      ...
      'Combine with n8n_get_node after finding the right node'
    ],
    pitfalls: [
    ...
    relatedTools: ['n8n_get_node to configure found nodes', 'n8n_search_templates to find workflow examples', 'n8n_validate_node to check configurations']
  }
};