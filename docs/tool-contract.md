...
- In dev, ready locators report `editReadiness.effectiveDecision=auto-sync-on-save`, `externalFilesystemEditAllowed=true`, `autoSyncOnSave=true`, `platformPreflightRequired=false` and `filesystemToolPolicy=edit-with-auto-sync-on-save`. `autoSyncRuntimeHealthVerified=false` keeps local eligibility separate from deployed controller health.
- In non-dev environments, ready locators report `effectiveDecision=inspect-only`, `externalFilesystemEditAllowed=false` and `autoSyncOnSave=false`. Additional platform approval fields are not part of locator responses.
...
- `synestra_workflow_file_validate.valid` and `safeToEdit` are local locator gates only. The response includes `safeToEditScope=local-locator-only`; use `validSyntax` for syntax-only results and `safeToEdit` for local path eligibility.
...
- Camel K, Debezium slot/publication and n8n DB-to-files parity are platform gates. They are checked through `~/repo/synestra-platform/scripts/n8n/preflight_n8n_camelk_recovery.sh`; Camel K operator/runtime, Debezium exporter and n8n Camel K workload sync planning uses `~/repo/synestra-platform/scripts/n8n/audit_n8n_camelk_upgrade_readiness.sh --json`; recovery planning uses `~/repo/synestra-platform/scripts/n8n/classify_n8n_recovery_candidates.sh`. None of these commands run from inside this MCP container.
- Platform `productionReadinessEvidence.gatewayStrict`, `fileLayerSafety.effectiveDecision`, `fileLayerSafety.blockers`, `fileLayerSafety.n8nDbContract`, `fileLayerSafety.blockerEvidence`, `externalFileEditAllowed`, `applyForbiddenReasons`, `nextActions`, `upgradePolicy`, `prodSyncImpact.sourceOfTruth`, `dbFilesBackfillDryRun.dirtyArtifactSafety`, `debeziumCdcFreshness.status` and `debeziumLogRisk.overallStatus` remain authoritative for production/runtime health and recovery decisions. Before claiming native+extensions readiness, verify `productionReadinessEvidence.canClaimProductionReadiness=true`, the deployed `gatewayStrict.synestraBuild.platformImageTag` and `sourceRef`, matching `gatewayStrict.buildAcceptance`, and clean `gatewayStrict.exportDiagnosticsAcceptance`. Local dev edit eligibility does not override DB contract, upgrade, recovery or CDC blockers. Fresh Debezium problem logs inside `N8N_CAMELK_LOG_ACTIVE_WINDOW_SEC` are active blockers until the exporter has a quiet window.
