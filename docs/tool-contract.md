...
- Platform `fileLayerSafety.synestraMcpBridge`, `fileLayerSafety.effectiveDecision`, `externalFileEditAllowed`, `applyForbiddenReasons`, `dbFilesBackfillDryRun.dirtyArtifactSafety`, `debeziumCdcFreshness.status` and `debeziumLogRisk.overallStatus` are authoritative for global file-layer readiness. A ready MCP locator is not enough when these fields show dirty workflow blockers or active/inconclusive/historical Debezium blockers. Fresh Debezium problem logs inside `N8N_CAMELK_LOG_ACTIVE_WINDOW_SEC` are active blockers until the exporter has a quiet window.
