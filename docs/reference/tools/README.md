...
| Tool | Summary |
|---|---|
| [n8n_code_node_test](./n8n_code_node_test.md) | Execute a Code-node-focused generated sub-workflow through the utility runner. `mode=node` and `mode=subgraph` target a Code node and optionally include nearby non-Code nodes. |
...
| [n8n_workflow_full_test](./n8n_workflow_full_test.md) | Execute a full workflow through n8n's native `/rest/workflows/:id/run` endpoint. Use this for editor-style/manual workflows when you want native full-test semantics. |
| [n8n_workflow_runner_test](./n8n_workflow_runner_test.md) | Execute a full workflow through the utility runner. Use this for manual-only or otherwise non-externally-triggerable workflows. Supports dry-run, synthetic input items, diagnostics, and full response mode. |
