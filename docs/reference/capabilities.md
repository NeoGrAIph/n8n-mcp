...
| CAP-workflow-files-tools | Workflow files tools (optional) | При доступном workflow root (`N8N_WORKFLOWS_ROOT`/`WORKFLOWS_ROOT` или дефолт `/workflows`) доступны инструменты чтения/записи файлов Code/Set нод и MCP resources для них. | How-to: [Work with Code/Set node files](../how-to/workflow-files-code-set.md) |
